% ! ptex2pdf -l -u           --shell-escape -output-directory=myout vol-all-天うつ浪
% ! cluttex --engine=uplatex --shell-escape -output-directory=myout vol-all-天うつ浪
...
line_length               = 28zw          , % TODO デフォルトはコメントアウトすべし
number_of_lines           = 11            , % TODO デフォルトはコメントアウトすべし
]{jlreq}
...
\ModifyPageStyle{myheadings}{
  ...
  odd_running_head      = {\其の番号} , % see also \SECTION
  even_running_head     = {\巻の番号} , % see also \CHAPTER
}
...
\setlength{\marginparwidth}{20mm}%% 傍注欄の大きさ%%%%% デバッグ用／本番は「空」

%%\newcommand*\原本頁[1][]{%
%%  \if\relax\detokenize\expandafter{\expanded{#1}}\relax%
%%  \else\marginpar{\hfill p-#1}%
%%  \fi
%%}
